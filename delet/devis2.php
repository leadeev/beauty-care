...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            <li class="active">
                <button data-toggle="modal" data-target="#modalAddDossier" class="btn btn-primary"><i
                        class="fa fa-plus-circle"></i>&nbsp; Nouveau dossier</button>
            </li>
        </ol>
    </section>
                        ...
                        <div class="row">
                            <div class="col-md-9">
                                <div class="form-group">
                                    ...
                                    <select name="patient" class="form-control select2" id="idPatient2">
                                        <option value="0" selected="" disabled="">-- vuillez choisie un patient --
                                        </option>
                                        <?php foreach( $patients as $patient ): ?>
                                        <option value="<?= $patient['idPatient'] ?>">
                                            <?php if( $patient['cin'] != '' ) echo '[ ' . $patient['cin'] . ' ] ' . $patient['prenom'] . '  ' . $patient['nom']; else echo $patient['prenom'] . '  ' . $patient['nom']; ?>
                                        </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <button style="margin-top: 5px;" id="btnFilterWithPatient"
                                        class="form-control btn-flat btn btn-primary">Chercher</button>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-5">
                                <div class="form-group">
                                    ...
                                    <input type="text" data-date-format="yyyy-mm-dd" placeholder="A partire de ..."
                                        class="form-control datepicker" id="dateDebut">
                                </div>
                            </div>
                            <div class="col-md-5">
                                <div class="form-group">
                                    <label for="dateFin" class="control-label">Jusqu'à</label>
                                    <input type="text" data-date-format="yyyy-mm-dd" placeholder="Jusqu'à ..."
                                        class="form-control datepicker" id="dateFin">
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label for="" class="control-label"></label>
                                    <button style="margin-top: 5px;" id="btnFilterWithdateCreation"
                                        class="form-control btn-flat btn btn-primary">Chercher</button>
                                </div>
                            </div>
                        </div>
    ...
    <div id="modalAddDossier" class="modal fade" data-backdrop="static" data-keyboard="false" role="dialog">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <form action="includes/functions/controller.php?action=addDossierPatient" id="FormAddDossier"
                    methode="post">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <select id="idPatient" name="idPatient" class="form-control select2">
                                <?php foreach( $patients as $patient ): ?>
                                <option value="<?= $patient['idPatient'] ?>">
                                    <?= '[ ' . $patient['cin'] . ' ] ' . $patient['prenom'] . ' ' . $patient['nom'] ?>
                                </option>
                                <?php endforeach ?>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="titreDossier" id="titreDossier" class="form-control"
                                placeholder="Entrez le titre de dossier...">
                            <span class="help-block"></span>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="text" disabled class="form-control datepicker" placeholder="Date de paiement"
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" id="btnAddDossier" class="btn btn-flat btn-success"> <i
                                class="fa fa-save"></i>&nbsp; Enregestrer </button>
                    </div>
                </form>
            </div>
        </div>
...
function addDossier() {
    loaderBtn('btnAddDossier', 'Chargement  &nbsp;' +
        '<i style="font-size: 13px;" class="fa fa-spinner fa-1x fa-spin"></i>');
        ...
        success: function(res) {
            ...
            $('#tblDossiers').DataTable({
                ...
                "fnRowCallback": function(nRow, aData, iDisplayIndex) {
                    ...
                    $("td:last", nRow).html(
                        '<a class="btn btn-info btn-sm btn-flat" href="dossierPatient.php?idDossier=' +
                        aData.idDossier +
                        '" ><i class="fa fa-eye"></i></a> <button class="btn btn-sm btn-danger btn-flat" onclick="deleteDossier(' +
                        aData.idDossier + ')"> <i class="fa fa-trash-o"></i></button>'
                    );
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        "render": function(data, type, row) {
                            return '<span class="text-blue">' + moment(data,
                                "YYYY-MM-DD HH:mm:ss").format("DD / MM / YYYY à hh:mm"); +
                            ' </span>';
                        }
                    },
                    {
                    ...
                    },
                    {
                        "data": "montantDossier",
                        "render": function(data, type, row) {
                            return '<span class="badge badge-no-circle bg-green">' + data +
                                ' DH</span>';
                        }
                    },
                    {
...
function getDossiersDateCreation() {
    ...
    $.ajax({
        ...
        url: 'http://localhost/sitecabinetDentaire/admin/includes/functions/controller.php?action=getDossiersDateCreation&dateDebut=' +
            dateDebut.value + '&dateFin=' + dateFin.value,
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                $.ajax({
                    ...
                    url: 'includes/functions/controller.php?action=deleteDossier&idDossier=' +
                        idDossier,
                    ...
                    success: function(data) {
                        if (data == 1) {
                            getDossiersPatient();
                            swal('Good !', 'Le dossier vous est bien supprimer !',
                                'success').insertQueueStep(1);
                            resolve();
                        } else {
                            swal('Erreur !', 'Impossible de supprimer ce dossier !',
                                'error').insertQueueStep(1);
                            resolve();
                        }
                    },
